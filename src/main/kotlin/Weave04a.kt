            ...
            extend {
                ...
                drawer.isolated {
                    ...
                    drawer.shadeStyle = shadeStyle {
                        ...
                        fragmentTransform = """
                                //float t = cos(c_screenPosition.y*0.01) * 0.5 + 0.5;
                                float t = smoothstep(0.0, 80.0, va_position.y) *
                                smoothstep(p_wefts, p_wefts-80.0, va_position.y);
                                
                                float c = abs(va_texCoord0.x - 0.5);
                                x_stroke.a *= smoothstep(-0.01, 0.01, t-c);
                                if (x_stroke.a <= 0.001) discard;
                            """.trimIndent()
                        parameter("wefts", wefts * 10.0)
                    ...
                    for (x in 0 until warps) {
                        ...
                        drawer.path(
                            ...
                            LineSegment3D(Vector3(x * 10.0, 0.0, 0.0), Vector3(x * 10.0, wefts * 10.0, 0.0)).path)
                    }
