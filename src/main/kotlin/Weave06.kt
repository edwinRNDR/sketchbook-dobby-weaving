    ...
    application {
        ...
        program {
            ...
            val weaveSettings = object {
                ...
                @DoubleParameter("contrast", 0.0, 10.0)
                var contrast = 1.0

                @ColorParameter("match color")
                var matchColor = ColorRGBa.WHITE
            ...
            val image = colorBuffer(warps * 10, wefts * 10, type = ColorType.FLOAT32)
            val s = image.shadow
            val typeSettings = object {
                ...
                fun loadImage() {
                    //val agb = ApproximateGaussianBlur()

                    openFileDialog { file ->
                        launch {
                            val timage = loadImage(file)
                            val rt = renderTarget(image.width, image.height, contentScale = 1.0) {
                                colorBuffer(image)
                            }
                ...
                drawer.isolated {
                    ...
                    drawer.isolated {
                        drawer.shadeStyle = shadeStyle {
                            fragmentPreamble = "$rgb2hsvPhrase"
                            fragmentTransform = """
                                //vec3 match = vec3(0.0, 1.0, 0.0);
                                vec3 match = vec3(1.0) - p_match.rgb;
                                vec3 matchHsv = rgb2hsv(match);
                                vec3 cmp = texture(p_image, vec2(p_line, 1.0 - va_texCoord0.y)).rgb;
                                vec3 cmpHsv = rgb2hsv(cmp);
                            
                                float hueMatch = clamp( 1.0 - abs(matchHsv.x - cmpHsv.x) * 1.14, 0.0, 1.0);
                                float satMatch = clamp(1.0 - abs(matchHsv.y - cmpHsv.y) * 1.0, 0.0, 1.0);
                            ...
                            parameter("line", 0.0)
                            parameter("contrast", weaveSettings.contrast)
                            parameter("match", weaveSettings.matchColor)
...
//                            )
                    ...
                    drawer.isolated {
                        drawer.shadeStyle = shadeStyle {
                            ...
                            fragmentTransform = """
                                ...
                                vec3 match = vec3(1.0) - p_match.rgb;
                                vec3 matchHsv = rgb2hsv(match);
                                vec3 cmp = texture(p_image, vec2(va_texCoord0.y, p_line)).rgb;
                                vec3 cmpHsv = rgb2hsv(cmp);
                        ...
                        for (y in 0 until wefts) {
                            drawer.shadeStyle?.parameter("line", 1.0 - y.toDouble() / wefts)
            ...
            keyboard.character.listen {
                ...
                if (c == 't') {
                    for (i in drafting.indices) {
                        drafting[i] =
                            drafting[i] * factor + (cos(i * 1 * PI / width) * (width) / 2.0 + (width) / 2).toInt()
                    }
                }
                ...
                if (c == 'g') {
                    for (i in threadling.indices) {
                        threadling[i] =
                            threadling[i] * factor + (cos(i * 1 * PI / width) * (width) / 2.0 + (width) / 2).toInt()
                    }
                }
